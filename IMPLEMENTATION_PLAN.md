...
| 32 | 2026-02-21 | **Red-Team Audit v2 Fixes — All High + Medium**: Fixed 60 findings from fresh audit. **Security**: (S-NEW-1) XXE fix — defusedxml for RSS parsing; (S-NEW-2) generic error messages, no exception leakage to client; (S-NEW-3/5/6/8/10) slack webhook, negative offset validation, SSE auth check, connection cleanup, HTML escaping. **Token/LLM**: (T-NEW-1) project_id/operation added to all 36 run_cli() calls across 22 files for cost tracking; (T-NEW-2) entity data capped at 30 attrs + 500 char values in insight prompts; (T-NEW-4) cache key normalization; (T-NEW-6) pre-serialized JSON schemas in 7 extractors; (T-NEW-7) classifier early-exit at 0.95. **UX**: (U-NEW-7) trapFocus() wired into all 3 modal dialogs; (U-NEW-2/3/5) disabled states, focus borders, button loading spinner; (U-NEW-13) tab error retry button; (U-NEW-19) floating label CSS; (U-NEW-21) 6 delete confirmations added. **Architecture**: (A-ARCH-1) central _TABLE_ENSURED reset in conftest.py; (A-ARCH-2) _conn() context manager with commit/rollback; (A-ARCH-8) busy_timeout 5s→10s; (A-PERF-4) overlap scan caching (5min TTL); (A-PERF-10) 2 FK indexes on extraction tables; (A-ARCH-9) AbortController guards on polling loops. Bundles rebuilt (785.1 KB JS, 168.1 KB CSS). 1700 tests passing. | ✅ Complete |
